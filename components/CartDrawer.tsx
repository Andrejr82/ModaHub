        ...
        {items.length === 0 ? (
          ...
          <>
              ...
              {items.map((item) => (
                <article key={item.productId} className="rounded-2xl border border-neutral-200 p-4">
                  <div className="grid grid-cols-[72px_1fr_auto] gap-3">
                    <div className={`h-20 rounded-2xl bg-gradient-to-br ${item.product.palette}`} aria-label={item.product.imageAlt} role="img" />
                    <div>
                    ...
                    <button type="button" onClick={() => onRemove(item.productId)} className="h-fit text-sm font-semibold text-clay focus:outline-none focus-visible:ring-2 focus-visible:ring-clay" aria-label={`Remover ${item.product.name}`}>Remover</button>
                  </div>
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <label className="text-sm font-semibold text-ink">Quantidade
                      <input type="number" min={1} max={item.product.stock} value={item.quantity} onChange={(event) => onUpdateQuantity(item.productId, Number(event.target.value))} className="ml-3 w-20 rounded-xl border border-neutral-300 px-3 py-2" />
                    </label>
            ...
            <div className="space-y-4 border-t border-neutral-200 p-5">
              ...
              <button type="button" className="w-full rounded-full bg-clay px-6 py-4 text-sm font-bold uppercase tracking-[0.2em] text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-clay">Finalizar compra</button>
              <button type="button" onClick={onClose} className="w-full rounded-full border border-ink px-6 py-3 text-sm font-bold uppercase tracking-[0.2em] text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-clay">Continuar comprando</button>
            </div>
          </>
        )}
      </aside>
